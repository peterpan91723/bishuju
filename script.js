const TABS_CONFIG = {
    ...
    dailyRsi70: { header: "RSI", format: v => v.value.toFixed(2), subFormat: v => `成交额 ${v.volumeFormatted} | EMA 9>21>55 间距扩张 | 量>MA20 | SAR多头 | CVD↑` },
    dailyRsi59: { header: "RSI", format: v => v.value.toFixed(2), subFormat: v => `成交额 ${v.volumeFormatted} | EMA 9>21>55 间距扩张 | 量>MA20 | SAR多头 | CVD↑` },
    dailySar: { header: "成交额 (USDT)", format: v => v.valueFormatted, subFormat: v => `SAR 翻多首根 | 量>MA20` },
};
...
function getColorClass(val, tab) {
    if (tab === "weeklyClosedVolume" || tab === "monthlyClosedVolume" || tab === "weeklyRsi" || tab === "dailyRsi70" || tab === "dailyRsi59" || tab === "dailySar") return "neutral";
